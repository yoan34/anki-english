...
demande_word_basic = lambda list, name: f"""
Peux-tu écrire la liste que je te fournis à la fin sous forme de tuple Python. Par exemple: ("to have", "avoir"),
saute une ligne à chaque tuple sans oublier la virgule. La liste: {list}
"""


demande_word_basic_sentence = lambda  list, name: f"""
Ecris un tuple Python qui invente une phrase à trou et le {name} conjugué pour la réponse et à l'infinitif dans l'autre partie du tuple.
("I ___ my job.", "left, leave"). Utilise le contexte de ma vie pour que les phrases à trou me soit familier. voici les verbes: {list}
"""
...
if __name__ == "__main__":
    ...
    from data.verbe import VERBES_ACTION

    print(VERBES_ACTION)
    result = demande_word_basic([], 'verbe')
    print(result)
    result = demande_word_basic_sentence(VERBES_ACTION[:52], 'verbe')
    print(result)
    result = demande_word_basic(VERBES_ACTION[52:], 'verbe')
    print(result)
    result = demande_word_basic_sentence('verbe')
    print(result)
